...
import { getTweet, type } from "react-tweet/api";
        ...
        >
          ...
          {tweet.user.verified ||
            (tweet.user.is_blue_verified && (
              <Verified className="inline ml-1 text-blue-500 size-4" />
            ))}
        </a>
...
export const TweetMedia = ({ tweet }) => (
  <div className="flex items-center justify-center flex-1">
    ...
    {tweet.photos && (
      <div className="relative flex gap-4 overflow-x-auto transform-gpu snap-x snap-mandatory">
        ...
        {tweet.photos.map((photo) => (
          <img
            ...
            alt={tweet.text}
            className="object-cover w-5/6 h-64 border shadow-sm shrink-0 snap-center snap-always rounded-xl"
          />
        ))}
        <div className="shrink-0 snap-center sm:w-2" />
      </div>
    ...
    {!tweet.video &&
      ...
      tweet?.card?.binding_values?.thumbnail_image_large?.image_value.url && (
        <img
          ...
          className="object-cover h-64 border shadow-sm rounded-xl"
        />
      )}
  </div>
);
